...
$g_translations = array (
		"zh_cn" => array (
            ...
            "Database Date (for MySql leave as default YYYY-MM-DD)" => "数据日期格式（MySQL默认为YYYY-MM-DD）",
            "HELP_PROJECT_NAME"=>"项目的名称，将在Reportico安装目录中的”projects“文件夹下创建具有此名称的文件夹",
            "HELP_PROJECT_TITLE"=>"出现在报告套件菜单顶部的标题，即项目的名称",
            "HELP_DATABASE_TYPE"=>"要报告的数据库类型",
            "HELP_DATABASE_HOST"=>"数据库所在主机的IP地址，本机使用127.0.0.1。对于SQLite数据库，保留为默认值。对于Oracle，Mysql，PostgreSQL数据库监听一个非标准端口，可以以HOSTNAME:PORT或IPADDRESS:PORT的形式指定",
            "HELP_DATABASE_NAME"=>"数据库的名称。对于SQLite数据库，请输入数据库文件的完整路径。",
            "HELP_DATABASE_USER"=>"数据库用户名",
            "HELP_DATABASE_PASSWORD"=>"数据库密码",
            "HELP_DB_ENCODING"=>"用于在数据库中存储字符的编码格式，UTF8将在英语区域和大多数其他情况下工作,您可以随时返回配置页面更改",
            ...
            "HELP_SAFE_MODE"=>"当打开时，设计模式将阻止输入自定义用户代码，分配和SQL语句（避免输入危险的PHP命令和SQL注入攻击）, 关闭此功能以启​​用对这些功能的访问。",
			)
		);
